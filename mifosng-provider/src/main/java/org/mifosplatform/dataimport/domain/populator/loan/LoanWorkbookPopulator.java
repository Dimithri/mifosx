package org.mifosplatform.dataimport.domain.populator.loan;
...
import org.mifosplatform.dataimport.data.loan.LoanProduct;
...
import org.mifosplatform.dataimport.domain.populator.ClientSheetPopulator;
import org.mifosplatform.dataimport.domain.populator.ExtrasSheetPopulator;
...
public class LoanWorkbookPopulator extends AbstractWorkbookPopulator {
    ...
    private Result setRules(Sheet worksheet) {
        ...
        try {
            ...
            CellRangeAddressList loanTypeRange = new CellRangeAddressList(1, SpreadsheetVersion.EXCEL97.getLastRowIndex(), LOAN_TYPE_COL,
                    LOAN_TYPE_COL);
            ...
            CellRangeAddressList loanOfficerRange = new CellRangeAddressList(1, SpreadsheetVersion.EXCEL97.getLastRowIndex(),
                    LOAN_OFFICER_NAME_COL, LOAN_OFFICER_NAME_COL);
            ...
            DataValidationConstraint loanTypeConstraint = validationHelper.createExplicitListConstraint(new String[] { "Individual",
                    "Group" });
            ...
            DataValidationConstraint submittedDateConstraint = validationHelper.createDateConstraint(
                    ...
                    "=IF(INDIRECT(CONCATENATE(\"START_DATE_\",$D1))>VLOOKUP($C1,$AQ$2:$AR$"
                            + (clientSheetPopulator.getClientsSize() + groupSheetPopulator.getGroupsSize() + 1)
                            + ",2,FALSE),INDIRECT(CONCATENATE(\"START_DATE_\",$D1)),VLOOKUP($C1,$AQ$2:$AR$"
            ...
            DataValidationConstraint disbursedDateConstraint = validationHelper.createDateConstraint(
            ...
            DataValidationConstraint principalConstraint = validationHelper.createDecimalConstraint(
                    DataValidationConstraint.OperatorType.BETWEEN, "=INDIRECT(CONCATENATE(\"MIN_PRINCIPAL_\",$D1))",
                    "=INDIRECT(CONCATENATE(\"MAX_PRINCIPAL_\",$D1))");
            DataValidationConstraint noOfRepaymentsConstraint = validationHelper.createIntegerConstraint(
                    DataValidationConstraint.OperatorType.BETWEEN, "=INDIRECT(CONCATENATE(\"MIN_REPAYMENT_\",$D1))",
                    "=INDIRECT(CONCATENATE(\"MAX_REPAYMENT_\",$D1))");
            DataValidationConstraint frequencyConstraint = validationHelper.createExplicitListConstraint(new String[] { "Days", "Weeks",
                    "Months" });
            DataValidationConstraint loanTermConstraint = validationHelper.createIntegerConstraint(
                    DataValidationConstraint.OperatorType.GREATER_OR_EQUAL, "=$L1*$M1", null);
            DataValidationConstraint interestFrequencyConstraint = validationHelper
                    .createFormulaListConstraint("INDIRECT(CONCATENATE(\"INTEREST_FREQUENCY_\",$D1))");
            DataValidationConstraint interestConstraint = validationHelper.createIntegerConstraint(
                    DataValidationConstraint.OperatorType.BETWEEN, "=INDIRECT(CONCATENATE(\"MIN_INTEREST_\",$D1))",
                    "=INDIRECT(CONCATENATE(\"MAX_INTEREST_\",$D1))");
            DataValidationConstraint amortizationConstraint = validationHelper.createExplicitListConstraint(new String[] {
                    "Equal principal payments", "Equal installments" });
            DataValidationConstraint interestMethodConstraint = validationHelper.createExplicitListConstraint(new String[] { "Flat",
                    "Declining Balance" });
            DataValidationConstraint interestCalculationPeriodConstraint = validationHelper.createExplicitListConstraint(new String[] {
            ...
            DataValidationConstraint repaymentStrategyConstraint = validationHelper.createExplicitListConstraint(new String[] {
                    "Mifos style", "Heavensfamily", "Creocore", "RBI (India)", "Principal Interest Penalties Fees Order",
            ...
            DataValidationConstraint graceOnPrincipalPaymentConstraint = validationHelper.createIntegerConstraint(
                    DataValidationConstraint.OperatorType.GREATER_OR_EQUAL, "0", null);
            DataValidationConstraint graceOnInterestPaymentConstraint = validationHelper.createIntegerConstraint(
                    DataValidationConstraint.OperatorType.GREATER_OR_EQUAL, "0", null);
            DataValidationConstraint graceOnInterestChargedConstraint = validationHelper.createIntegerConstraint(
                    DataValidationConstraint.OperatorType.GREATER_OR_EQUAL, "0", null);
            DataValidationConstraint lastRepaymentDateConstraint = validationHelper.createDateConstraint(
                    DataValidationConstraint.OperatorType.BETWEEN, "=$H1", "=TODAY()", "dd/mm/yy");
            ...
            DataValidation arrearsToleranceValidation = validationHelper
                    .createValidation(arrearsToleranceConstraint, arrearsToleranceRange);
    ...
    private Result setDefaults(Sheet worksheet) {
        ...
        try {
            for (Integer rowNo = 1; rowNo < 1000; rowNo++) {
                ...
                writeFormula(FUND_NAME_COL, row, "IF(ISERROR(INDIRECT(CONCATENATE(\"FUND_\",$D" + (rowNo + 1)
                        + "))),\"\",INDIRECT(CONCATENATE(\"FUND_\",$D" + (rowNo + 1) + ")))");
                writeFormula(PRINCIPAL_COL, row, "IF(ISERROR(INDIRECT(CONCATENATE(\"PRINCIPAL_\",$D" + (rowNo + 1)
                        + "))),\"\",INDIRECT(CONCATENATE(\"PRINCIPAL_\",$D" + (rowNo + 1) + ")))");
                writeFormula(REPAID_EVERY_COL, row, "IF(ISERROR(INDIRECT(CONCATENATE(\"REPAYMENT_EVERY_\",$D" + (rowNo + 1)
                        + "))),\"\",INDIRECT(CONCATENATE(\"REPAYMENT_EVERY_\",$D" + (rowNo + 1) + ")))");
                writeFormula(REPAID_EVERY_FREQUENCY_COL, row, "IF(ISERROR(INDIRECT(CONCATENATE(\"REPAYMENT_FREQUENCY_\",$D" + (rowNo + 1)
                        + "))),\"\",INDIRECT(CONCATENATE(\"REPAYMENT_FREQUENCY_\",$D" + (rowNo + 1) + ")))");
                writeFormula(NO_OF_REPAYMENTS_COL, row, "IF(ISERROR(INDIRECT(CONCATENATE(\"NO_REPAYMENT_\",$D" + (rowNo + 1)
                        + "))),\"\",INDIRECT(CONCATENATE(\"NO_REPAYMENT_\",$D" + (rowNo + 1) + ")))");
                writeFormula(LOAN_TERM_COL, row, "IF(ISERROR($L" + (rowNo + 1) + "*$M" + (rowNo + 1) + "),\"\",$L" + (rowNo + 1) + "*$M"
                        + (rowNo + 1) + ")");
                writeFormula(LOAN_TERM_FREQUENCY_COL, row, "$N" + (rowNo + 1));
                writeFormula(NOMINAL_INTEREST_RATE_FREQUENCY_COL, row, "IF(ISERROR(INDIRECT(CONCATENATE(\"INTEREST_FREQUENCY_\",$D"
                        + (rowNo + 1) + "))),\"\",INDIRECT(CONCATENATE(\"INTEREST_FREQUENCY_\",$D" + (rowNo + 1) + ")))");
                writeFormula(NOMINAL_INTEREST_RATE_COL, row, "IF(ISERROR(INDIRECT(CONCATENATE(\"INTEREST_\",$D" + (rowNo + 1)
                        + "))),\"\",INDIRECT(CONCATENATE(\"INTEREST_\",$D" + (rowNo + 1) + ")))");
                writeFormula(AMORTIZATION_COL, row, "IF(ISERROR(INDIRECT(CONCATENATE(\"AMORTIZATION_\",$D" + (rowNo + 1)
                        + "))),\"\",INDIRECT(CONCATENATE(\"AMORTIZATION_\",$D" + (rowNo + 1) + ")))");
                writeFormula(INTEREST_METHOD_COL, row, "IF(ISERROR(INDIRECT(CONCATENATE(\"INTEREST_TYPE_\",$D" + (rowNo + 1)
                        + "))),\"\",INDIRECT(CONCATENATE(\"INTEREST_TYPE_\",$D" + (rowNo + 1) + ")))");
                writeFormula(INTEREST_CALCULATION_PERIOD_COL, row, "IF(ISERROR(INDIRECT(CONCATENATE(\"INTEREST_CALCULATION_\",$D"
                        + (rowNo + 1) + "))),\"\",INDIRECT(CONCATENATE(\"INTEREST_CALCULATION_\",$D" + (rowNo + 1) + ")))");
                writeFormula(ARREARS_TOLERANCE_COL, row, "IF(ISERROR(INDIRECT(CONCATENATE(\"ARREARS_TOLERANCE_\",$D" + (rowNo + 1)
                        + "))),\"\",INDIRECT(CONCATENATE(\"ARREARS_TOLERANCE_\",$D" + (rowNo + 1) + ")))");
                writeFormula(REPAYMENT_STRATEGY_COL, row, "IF(ISERROR(INDIRECT(CONCATENATE(\"STRATEGY_\",$D" + (rowNo + 1)
                        + "))),\"\",INDIRECT(CONCATENATE(\"STRATEGY_\",$D" + (rowNo + 1) + ")))");
                writeFormula(GRACE_ON_PRINCIPAL_PAYMENT_COL, row, "IF(ISERROR(INDIRECT(CONCATENATE(\"GRACE_PRINCIPAL_\",$D" + (rowNo + 1)
                        + "))),\"\",INDIRECT(CONCATENATE(\"GRACE_PRINCIPAL_\",$D" + (rowNo + 1) + ")))");
                writeFormula(GRACE_ON_INTEREST_PAYMENT_COL, row, "IF(ISERROR(INDIRECT(CONCATENATE(\"GRACE_INTEREST_PAYMENT_\",$D"
                        + (rowNo + 1) + "))),\"\",INDIRECT(CONCATENATE(\"GRACE_INTEREST_PAYMENT_\",$D" + (rowNo + 1) + ")))");
                writeFormula(GRACE_ON_INTEREST_CHARGED_COL, row, "IF(ISERROR(INDIRECT(CONCATENATE(\"GRACE_INTEREST_CHARGED_\",$D"
                        + (rowNo + 1) + "))),\"\",INDIRECT(CONCATENATE(\"GRACE_INTEREST_CHARGED_\",$D" + (rowNo + 1) + ")))");
            }
        } catch (RuntimeException re) {
            result.addError(re.getMessage());
        }
    ...
    private void setNames(Sheet worksheet) {
        ...
        for (Integer i = 0; i < products.size(); i++) {
            ...
            if (products.get(i).getFundName() != null) fundName.setRefersToFormula("Products!$C$" + (i + 2));
