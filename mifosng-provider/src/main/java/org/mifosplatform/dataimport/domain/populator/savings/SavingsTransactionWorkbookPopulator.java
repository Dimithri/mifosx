package org.mifosplatform.dataimport.domain.populator.savings;
...
import org.mifosplatform.dataimport.data.loan.CompactLoan;
import org.mifosplatform.dataimport.data.savings.CompactSavingsAccount;
...
public class SavingsTransactionWorkbookPopulator extends AbstractWorkbookPopulator {
    ...
    public SavingsTransactionWorkbookPopulator(final SavingsAccountReadPlatformService savingsAccountReadPlatformService,
            OfficeSheetPopulator officeSheetPopulator, ClientSheetPopulator clientSheetPopulator, ExtrasSheetPopulator extrasSheetPopulator) {
    ...
    private Result setRules(Sheet worksheet) {
        ...
        try {
            ...
            CellRangeAddressList accountNumberRange = new CellRangeAddressList(1, SpreadsheetVersion.EXCEL97.getLastRowIndex(),
                    SAVINGS_ACCOUNT_NO_COL, SAVINGS_ACCOUNT_NO_COL);
            ...
            DataValidationConstraint accountNumberConstraint = validationHelper
                    .createFormulaListConstraint("INDIRECT(CONCATENATE(\"Account_\",SUBSTITUTE(SUBSTITUTE(SUBSTITUTE($B1,\" \",\"_\"),\"(\",\"_\"),\")\",\"_\")))");
            DataValidationConstraint transactionTypeConstraint = validationHelper.createExplicitListConstraint(new String[] { "Withdrawal",
                    "Deposit" });
            ...
            DataValidationConstraint transactionDateConstraint = validationHelper.createDateConstraint(
                    DataValidationConstraint.OperatorType.BETWEEN, "=VLOOKUP($C1,$Q$2:$T$" + (savings.size() + 1) + ",4,FALSE)",
                    "=TODAY()", "dd/mm/yy");
    ...
    private void setDefaults(Sheet worksheet) {
        try {
            for (Integer rowNo = 1; rowNo < 3000; rowNo++) {
                ...
                writeFormula(PRODUCT_COL, row, "IF(ISERROR(VLOOKUP($C" + (rowNo + 1) + ",$Q$2:$S$" + (savings.size() + 1)
                        + ",2,FALSE)),\"\",VLOOKUP($C" + (rowNo + 1) + ",$Q$2:$S$" + (savings.size() + 1) + ",2,FALSE))");
                writeFormula(OPENING_BALANCE_COL, row, "IF(ISERROR(VLOOKUP($C" + (rowNo + 1) + ",$Q$2:$S$" + (savings.size() + 1)
                        + ",3,FALSE)),\"\",VLOOKUP($C" + (rowNo + 1) + ",$Q$2:$S$" + (savings.size() + 1) + ",3,FALSE))");
            }
        } catch (Exception e) {
            logger.error(e.getMessage());
        }
    ...
    private void setNames(Sheet worksheet) {
        ...
        for (int j = 0; j < clientsWithActiveSavings.size(); j++) {
            ...
            name.setNameName("Account_" + clientsWithActiveSavings.get(j).replaceAll(" ", "_") + "_" + clientIdsWithActiveSavings.get(j)
                    + "_");
            name.setRefersToFormula("SavingsTransaction!$Q$" + clientNameToBeginEndIndexes.get(clientsWithActiveSavings.get(j))[0] + ":$Q$"
