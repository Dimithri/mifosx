...
public class PersonnelSheetPopulator extends AbstractWorkbookPopulator {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(PersonnelSheetPopulator.class);
	...
	public PersonnelSheetPopulator(Boolean onlyLoanOfficers,
	...
	private void populateStaffByOfficeName(Sheet staffSheet) {
		...
		for (Office office : offices) {
			...
			writeString(OFFICE_NAME_COL, row, office.getName().trim()
					.replaceAll("[ )(]", "_"));

			ArrayList<String> fullStaffList = getStaffList(office
					.getHierarchy());
			...
			if (!fullStaffList.isEmpty()) {
				...
				officeNameToBeginEndIndexesOfStaff.put(officeIndex++,
	...
	private ArrayList<String> getStaffList(String hierarchy) {
		...
		String[] officeIds = hierarchy.substring(1, hierarchyLength).split(
				"\\.");
		String headOffice = offices.get(0).getName().trim()
				.replaceAll("[ )(]", "_");
		...
		for (int i = 0; i < officeIds.length; i++) {
			String officeName = getOfficeNameFromOfficeId(Integer
					.parseInt(officeIds[i]));
