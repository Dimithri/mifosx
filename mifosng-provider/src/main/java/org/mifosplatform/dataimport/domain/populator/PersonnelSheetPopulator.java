...
import org.mifosplatform.dataimport.data.Personnel;
...
public class PersonnelSheetPopulator extends AbstractWorkbookPopulator {
    ...
    public Result downloadAndParse() {
        ...
        try {
            ...
            // Parse Staff Data into local variables
            ...
            /*
             * client.createAuthToken(); personnel = new ArrayList<Personnel>();
             * content = client.get("staff?limit=-1"); parseStaff(); content =
             * client.get("offices?limit=-1"); parseOffices();
             */
    ...
    private void parseStaff() {
         ...
         * JsonParser().parse(content); JsonArray array = json.getAsJsonArray();
         * Iterator<JsonElement> iterator = array.iterator(); staffNameToStaffId
         * = new HashMap<String, Integer>(); while (iterator.hasNext()) { json =
         * iterator.next(); Personnel person = gson.fromJson(json,
         * Personnel.class); if (!onlyLoanOfficers) personnel.add(person); else
         * { if (person.isLoanOfficer()) personnel.add(person); }
         * staffNameToStaffId.put(person.getName(), person.getId()); }
         */
    }
    ...
    private void parseOffices() {
        /*
         * offices = new ArrayList<Office>(); JsonElement json = new
         * JsonParser().parse(content); JsonArray array = json.getAsJsonArray();
         * Iterator<JsonElement> iterator = array.iterator();
         * officeIdToOfficeName = new HashMap<Integer, String>(); while
         * (iterator.hasNext()) { json = iterator.next(); Office office = new
         * Gson().fromJson(json, Office.class);
         * officeIdToOfficeName.put(office.getId(), office.getName().trim()
         * .replaceAll("[ )(]", "_")); offices.add(office); }
         */
    }
