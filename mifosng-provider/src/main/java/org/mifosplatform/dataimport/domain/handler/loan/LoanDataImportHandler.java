package org.mifosplatform.dataimport.domain.handler.loan;
...
import org.mifosplatform.dataimport.data.Approval;
import org.mifosplatform.dataimport.data.Transaction;
import org.mifosplatform.dataimport.data.loan.GroupLoan;
import org.mifosplatform.dataimport.data.loan.Loan;
import org.mifosplatform.dataimport.data.loan.LoanDisbursal;
    ...
    public Result upload() {
        ...
        for (int i = 0; i < loans.size(); i++) {
            ...
            try {
                ...
                if (progressLevel <= 2) progressLevel = uploadLoanDisbursal(loanId, i);
    ...
    private Integer uploadLoanApproval(String loanId, int rowIndex) {
        if (approvalDates.get(rowIndex) != null) {
            ...
            final CommandWrapper commandRequest = new CommandWrapperBuilder().withJson(payload)
                    .approveLoanApplication(Long.parseLong(loanId, 10)).build();
