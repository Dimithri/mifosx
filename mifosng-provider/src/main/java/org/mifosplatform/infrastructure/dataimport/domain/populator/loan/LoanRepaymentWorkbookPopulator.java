...
public class LoanRepaymentWorkbookPopulator extends AbstractWorkbookPopulator {
    ...
    private Result populateLoansTable(Sheet loanRepaymentSheet) {
        ...
        try {
            for (CompactLoan loan : loans) {
                ...
                ArrayList<Integer> actualDisbursementDate = loan.getTimeline().getActualDisbursementDate();
                if (actualDisbursementDate.size() == 3)
                    writeDate(LOOKUP_LOAN_DISBURSEMENT_DATE_COL, row, actualDisbursementDate.get(2) + "/" + actualDisbursementDate.get(1)
                            + "/" + actualDisbursementDate.get(0), dateCellStyle);
            }
        } catch (Exception e) {
            result.addError(e.getMessage());
        }
    ...
    private Result setRules(Sheet worksheet) {
        ...
        try {
            ...
            CellRangeAddressList accountNumberRange = new CellRangeAddressList(1, SpreadsheetVersion.EXCEL97.getLastRowIndex(),
                    LOAN_ACCOUNT_NO_COL, LOAN_ACCOUNT_NO_COL);
            ...
            CellRangeAddressList repaymentDateRange = new CellRangeAddressList(1, SpreadsheetVersion.EXCEL97.getLastRowIndex(),
                    REPAID_ON_DATE_COL, REPAID_ON_DATE_COL);
            ...
            DataValidationConstraint repaymentDateConstraint = validationHelper.createDateConstraint(
                    DataValidationConstraint.OperatorType.BETWEEN, "=VLOOKUP($C1,$P$2:$S$" + (loans.size() + 1) + ",4,FALSE)", "=TODAY()",
                    "dd/mm/yy");
    ...
    private void setDefaults(Sheet worksheet) {
        try {
            for (Integer rowNo = 1; rowNo < 3000; rowNo++) {
                ...
                writeFormula(PRODUCT_COL, row, "IF(ISERROR(VLOOKUP($C" + (rowNo + 1) + ",$P$2:$R$" + (loans.size() + 1)
                        + ",2,FALSE)),\"\",VLOOKUP($C" + (rowNo + 1) + ",$P$2:$R$" + (loans.size() + 1) + ",2,FALSE))");
                writeFormula(PRINCIPAL_COL, row, "IF(ISERROR(VLOOKUP($C" + (rowNo + 1) + ",$P$2:$R$" + (loans.size() + 1)
                        + ",3,FALSE)),\"\",VLOOKUP($C" + (rowNo + 1) + ",$P$2:$R$" + (loans.size() + 1) + ",3,FALSE))");
            }
        } catch (Exception e) {
            logger.error(e.getMessage());
        }
    ...
    private void setNames(Sheet worksheet) {
        ...
        for (int j = 0; j < clientsWithActiveLoans.size(); j++) {
            ...
            name.setRefersToFormula("LoanRepayment!$P$" + clientNameToBeginEndIndexes.get(clientsWithActiveLoans.get(j))[0] + ":$P$"
