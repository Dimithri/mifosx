package org.mifosplatform.infrastructure.dataimport.domain.handler.group;
...
import org.mifosplatform.infrastructure.dataimport.data.group.Group;
import org.mifosplatform.infrastructure.dataimport.data.group.Meeting;
import org.mifosplatform.infrastructure.dataimport.data.group.WeeklyMeeting;
...
public class GroupDataImportHandler extends AbstractDataImportHandler {
    ...
    public Result upload() {
        ...
        for (int i = 0; i < groups.size(); i++) {
            ...
            try {
                ...
                if (meetings.get(i) != null) progressLevel = uploadGroupMeeting(groupId, i);
                ...
                if (progressLevel == 0)
                    status = "Creation";
                else if (progressLevel == 1) status = "Meeting";
                statusCell.setCellValue(status + " failed.");
                statusCell.setCellStyle(getCellStyle(workbook, IndexedColors.RED));

                if (progressLevel > 0) row.createCell(GROUP_ID_COL).setCellValue(Integer.parseInt(groupId));
    ...
    private Integer uploadGroupMeeting(String groupId, int rowIndex) {
        ...
        CommandWrapperBuilder resourceDetailsBuilder = new CommandWrapperBuilder();
        resourceDetailsBuilder.withGroupId(Long.parseLong(entityId, 10));
        final CommandWrapper resourceDetails = resourceDetailsBuilder.build();

        final CommandWrapper commandRequest = new CommandWrapperBuilder()
                .createCalendar(resourceDetails, entityType, Long.parseLong(entityId, 10)).withJson(payload).build();
